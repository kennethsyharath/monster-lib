...
export class Character {
  ...
  constructor(stats: [string, number][], actions: [string, Action][]) {
    stats.forEach((cur) => {
      const [statId, value] = cur;
      this.stats.set(statId, value);
    });
    actions.forEach((cur) => {
      const [actionId, action] = cur;
  ...
  public setStat(statId: string, value: number, overrideExistenceCheck: boolean = false) {
