...
test('can assign stat', () => {
  const char = new Character([['strength', 10]], []);
...
test('can assign actions', () => {
  const action = new Action([['toHit', new Roll(0, new DndOutcomeRuleset(), 20)]]);

  const char = new Character([], [['attack', action]]);
...
test('can perform given action', () => {
  const action = new Action([
    ['toHit', new Roll(0, new DndOutcomeRuleset(), 20)],
    ['damage', new Roll(0, new DndOutcomeRuleset(), 8)],
  ]);
